...
local mcp_router = require('mcp.router')
...
local function requested_probe_mode()
    return vim.env.ACP_LIVE_PROBE_MODE or 'balanced'
end
...
local function instrumented_rpc_factory(opts)
    local client = require('acp.rpc').new(opts)
...
local ok, err = xpcall(function()
    ...
    assert(
        ...
        string.format('mcp.nvim did not advertise neovim/terminal/create: %s', vim.inspect(tool_names))
    )
    assert(
        vim.tbl_contains(tool_names, 'neovim/terminal/output'),
        string.format('mcp.nvim did not advertise neovim/terminal/output: %s', vim.inspect(tool_names))
    )
    assert(
        vim.tbl_contains(tool_names, 'neovim/terminal/wait'),
        string.format('mcp.nvim did not advertise neovim/terminal/wait: %s', vim.inspect(tool_names))
    )
    assert(
        vim.tbl_contains(tool_names, 'neovim/terminal/release'),
        string.format('mcp.nvim did not advertise neovim/terminal/release: %s', vim.inspect(tool_names))
    )
...
transport._set_rpc_factory(require('acp.rpc').new)
