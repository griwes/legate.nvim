...
- disk-backed ACP filesystem requests use the active local session workspace rather than an agent-supplied root, canonicalize every allowed root and target, reject broken links and symlinks that escape those roots, and validate missing targets through their nearest existing parent; ACP-edited buffers retain an exact canonical-target write guard until the later real buffer write succeeds
...
- inbound ACP requests have bounded active count/bytes, per-request deadlines, exact lifecycle accounting, and cancellation hooks for pending permission and terminal state
- ACP terminal wait requests complete asynchronously, release cancels outstanding native and Terminalia waits, and native release escalates termination after a bounded grace period instead of blocking Neovim or orphaning a TERM-ignoring process
...
- `scripts/ci/run.sh` runs the same Stylua and test gates used by the GitHub
