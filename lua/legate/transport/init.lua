...
local function active_request_session(params)
    ...
    local current_transport_remote_id = current_session ~= nil and continuity.transport_remote_id(current_session)
        or nil
    ...
    local waiting_transport_remote_id = waiting_session ~= nil and continuity.transport_remote_id(waiting_session)
        or nil
...
local function reveal_inline_approval(current_session)
    ...
    if selected_session ~= nil and selected_session.id ~= current_session.id then
        ...
        if prompt ~= nil then
            continuity.set_draft_prompt(selected_session, prompt)
        end

        continuity.select(current_session.id)
...
local function cancel_pending_permission(current_session)
    ...
    for _, pending_permission in ipairs(pending_permissions) do
        if current_session ~= nil and pending_permission.local_session_id ~= current_session.id then
            ...
            local pending_session = continuity.get(pending_permission.local_session_id)
...
local function apply_update(current_session, update)
    continuity.apply_update(current_session, update)
    rerender(current_session)
end
...
local function finish_turn(current_session, stop_reason)
    continuity.finish_prompt(current_session, stop_reason)
    rerender(current_session)
end
...
local function initialize(current_session)
    ...
    continuity.set_agent_info(current_session, result.agentInfo)
end
...
local function establish_session(current_session, opts)
    ...
    local previous_transport_remote_id = continuity.transport_remote_id(current_session)
    if force_load and previous_transport_remote_id == nil then
        ...
        if previous_transport_remote_id ~= nil then
            continuity.set_transport_remote_id(
                current_session,
    ...
    if force_new then
        continuity.clear_remote_id(current_session)
        state.bound_local_session_id = nil
        state.bound_remote_session_id = nil
        state.active_session = nil
        state.loaded_existing_session = false
    ...
    if not force_new and previous_transport_remote_id ~= nil then
        if state.agent_capabilities ~= nil and state.agent_capabilities.loadSession then
            ...
            if rpc_error == nil then
                ...
                continuity.set_remote_sync_state(current_session, 'loaded')
                continuity.set_cwd(current_session, cwd)
                continuity.set_available_commands(current_session, {})
                continuity.set_config_options(current_session, result.configOptions or {})
            ...
            state.active_session = nil
            state.loaded_existing_session = false
            continuity.set_transport_remote_id(current_session, nil, 'load_failed', rpc_error.message)
        ...
        elseif force_load then
            ...
            state.active_session = nil
            state.loaded_existing_session = false
            continuity.set_transport_remote_id(current_session, nil, 'load_failed', message)
            error(message)
        end
    end
    ...
    if rpc_error ~= nil then
        ...
        elseif force_new then
            continuity.clear_remote_id(current_session, previous_remote_sync_state, previous_remote_sync_error)
        end
    ...
    continuity.set_remote_id(current_session, result.sessionId, 'created')
    continuity.set_cwd(current_session, cwd)
    continuity.set_config_options(current_session, result.configOptions or {})
    apply_adapter_config_option_overrides(current_session)
    continuity.set_available_commands(current_session, {})
