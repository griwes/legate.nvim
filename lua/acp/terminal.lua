...
---@class acp.TerminalModule
local M = {}

local REQUEST_ERROR_CODE = -32000

---@class acp.NativeTerminalState
---@field handle acp.TerminalHandle
---@field system vim.SystemObj
---@field output string
---@field truncated boolean
---@field output_byte_limit integer?
---@field exit_status acp.TerminalExitStatus?
---@field preview_bufnr integer?

---@type table<string, acp.NativeTerminalState>
local terminals = {}
---@class acp.TerminalManagerTerminalState
---@field handle acp.TerminalHandle
---@field output_byte_limit integer?

---@type table<string, acp.TerminalManagerTerminalState>
local terminal_manager_terminals = {}
local next_terminal_ordinal = 1
local terminal_manager_module_name = 'terminal_manager'
...
---@param state acp.NativeTerminalState
...
---@param handle acp.TerminalHandle
---@return acp.TerminalManagerTerminalState?, table?
...
local function terminal_manager_api()
    ...
    if not ok then
        error('ACP terminal_manager backend requires terminal-manager.nvim to be installed')
    end
...
---@param handle acp.TerminalHandle
---@return acp.NativeTerminalState?, table?
...
---@return acp.TerminalWaitForExitResponse?, table?
...
local native_backend = {
    ...
    reveal = function(handle)
        ...
        if state.preview_bufnr == nil or not vim.api.nvim_buf_is_valid(state.preview_bufnr) then
            ...
            vim.api.nvim_buf_set_name(bufnr, string.format('ACPTerminal:%s', handle.id))
...
---@return acp.TerminalBackend
function M.resolve()
    if config.get().terminal_backend == 'native' then
        return native_backend
    end

    if config.get().terminal_backend == 'terminal_manager' then
...
---@param params acp.TerminalCreateRequest
---@return acp.TerminalCreateResponse?, table?
